...
export const tenantStatusEnum = ['ACTIVE', 'INACTIVE', 'PENDING', 'BLACKLISTED'] as const satisfies readonly DbTenantStatus[];
export const leaseStatusEnum = ['ACTIVE', 'INACTIVE', 'EXPIRED', 'TERMINATED'] as const satisfies readonly DbLeaseStatus[];
...
export const baseTenantSchema = z.object({
  ...
  outstanding_balance: z.number().min(0).nullish().transform(val => val ?? 0),
  last_payment_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Invalid date format').nullish(),
  next_payment_due: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Invalid date format').nullish(),
  ...
  status_history: z.array(z.object({
    ...
    changed_at: z.string().nullish().transform(val => val ?? new Date().toISOString()),
