...
<script lang="ts">
    ...
    import { leaseStatusEnum, leaseTypeEnum } from './formSchema';
    ...
    function generateRandomLease() {
      const randomLease = {
        ...
        leaseStatus: Object.values(leaseStatusEnum.Values)[
      Math.floor(Math.random() * Object.values(leaseStatusEnum.Values).length)
    ],
    leaseType: Object.values(leaseTypeEnum.Values)[
      Math.floor(Math.random() * Object.values(leaseTypeEnum.Values).length)
    ],
