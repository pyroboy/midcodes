...
export const actions = {
    updatePayment: async ({ request, locals: { safeGetSession, supabase }}: import('./$types').RequestEvent) => {
        const sessionInfo = await safeGetSession();
        if (!sessionInfo.session ) {
    ...
    clearExpired: async ({ request, locals: { safeGetSession,supabase } }: import('./$types').RequestEvent) => {
    ...
    deleteAllArchived: async ({ request, locals: { supabase } }: import('./$types').RequestEvent) => {
        const formData = await request.formData();
