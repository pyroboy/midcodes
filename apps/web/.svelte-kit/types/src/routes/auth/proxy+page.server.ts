...
export const load = async ({ locals: { safeGetSession }, url }: Parameters<PageServerLoad>[0]) => {
...
export const actions = {
    signin: async ({ request, locals: { supabase }, cookies }: import('./$types').RequestEvent) => {
    ...
    signup: async ({ request, url, locals: { supabase } }: import('./$types').RequestEvent) => {
        const formData = await request.formData();
        const email = formData.get('email') as string;
        const password = formData.get('password') as string;
