...
const load = async ({ locals: { safeGetSession, supabase } }) => {
  ...
  const { data: recentCards, error: cardsError } = await supabase.from("idcards").select("id, template_id, front_image, back_image, created_at, data").eq("org_id", effectiveOrgId).order("created_at", { ascending: false }).limit(5);
  const { count: totalCards, error: countError } = await supabase.from("idcards").select("*", { count: "exact", head: true }).eq("org_id", effectiveOrgId);
  if (cardsError) {
