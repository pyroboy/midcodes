...
const load = async ({ locals: { supabase, getSession } }) => {
  ...
  const { data: readings, error: readingsError } = await supabase.from("readings").select(`
      meter_id,
      reading_date,
      reading_value
    `).eq("org_id", profile.org_id);
  if (readingsError) throw error(500, "Error fetching readings");
  const { data: availableReadingDates, error: datesError } = await supabase.from("readings").select("reading_date").eq("org_id", profile.org_id).order("reading_date");
  if (datesError) throw error(500, "Error fetching reading dates");
  const { data: tenantCounts, error: tenantsError } = await supabase.from("leases").select(`
      rental_unit_id,
