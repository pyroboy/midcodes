...
import { r as rental_unitSchema } from "../../../../chunks/formSchema8.js";
...
const actions = {
  create: async ({ request, locals: { supabase } }) => {
    ...
    if (!form.data.property_id || !form.data.floor_id) {
      ...
      return fail(400, {
        ...
        errors: {
          property_id: !form.data.property_id ? "Property is required" : void 0,
          floor_id: !form.data.floor_id ? "Floor is required" : void 0
        }
      });
    }
    const existingRental_UnitResponse = await supabase.from("rental_unit").select("id").eq("floor_id", form.data.floor_id).eq("number", form.data.number).single();
    ...
    const insertResponse = await supabase.from("rental_unit").insert({
      property_id: form.data.property_id,
      floor_id: form.data.floor_id,
      name: form.data.name,
  ...
  update: async ({ request, locals: { supabase } }) => {
    ...
    const existingRental_UnitResponse = await supabase.from("rental_unit").select("id").eq("floor_id", form.data.floor_id).eq("number", form.data.number).neq("id", form.data.id).single();
    if (existingRental_UnitResponse.data) {
      return fail(400, {
        form,
        message: "Rental_unit number already exists on this floor"
    ...
    const updateResponse = await supabase.from("rental_unit").update({
      property_id: form.data.property_id,
      floor_id: form.data.floor_id,
      name: form.data.name,
      number: form.data.number,
  ...
  delete: async ({ request, locals: { supabase } }) => {
    ...
    const deleteResponse = await supabase.from("rental_unit").delete().eq("id", rental_unitId);
