	...
	if (url.searchParams.get('format') === 'pdf') {
		...
		return new Response(pdfBytes as any, {
			headers: {
				...
				'content-disposition': 'attachment; filename="silog-docs.pdf"'
			}
		});
	}
...
function cleanSvelteContent(html: string): string {
	return html
		...
		.replace(/\{@html[\s\S]*?\}/gi, '') // Remove dynamic HTML injection
        .replace(/\{[^}]+\}/g, '') // Remove simple interpolations if they are just variable names, though this is risky. 
        // ID-Gen didn't have the last one, but let's stick to ID-gen's logic exactly to be safe.
		.replace(/&nbsp;/g, ' ')
		.trim();
}
...
async function generatePdf(modules: { markdown: string; html: string }[]): Promise<Uint8Array> {
	...
	const drawFooter = (p: PDFPage, n: number) => {
		p.drawText(`Silog Business Docs • Page ${n}`, { x: margin, y: 25, size: 8, font: fontRegular, color: colors.muted });
