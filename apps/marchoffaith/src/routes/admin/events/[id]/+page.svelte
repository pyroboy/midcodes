<script lang="ts">
	...
	let titleValue = $state(data.event.title);
	...
	let imageUrlValue = $state(data.event.imageUrl ?? '');
</script>

<svelte:head>
	<title>Edit {data.event.title} - MOFI Admin</title>
</svelte:head>
	...
	<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
		<div class="space-y-5">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div class="space-y-1.5">
					...
					<input
						...
						type="datetime-local"
						required
						value={toDatetimeLocal(data.event.date)}
						class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm"
					/>
				</div>
				<div class="space-y-1.5">
				...
				<div class="space-y-1.5">
					...
					>
						...
						{#each data.churches as church}
							<option value={church.id} selected={data.event.churchId === church.id}>{church.name}</option>
						{/each}
					</select>
				</div>
			</div>
			...
			<div class="flex items-center gap-2 mt-2">
				<input id="isPublished" name="isPublished" type="checkbox" checked={data.event.isPublished} class="rounded border-gray-300 text-red-600 focus:ring-red-500" />
