<script lang="ts">
	...
	let photoUrlValue = $state(form?.photoUrl ?? data.pastor.photoUrl ?? '');

	let name = $state(form?.name ?? data.pastor.name);
	let slug = $state(form?.slug ?? data.pastor.slug);
...
<svelte:head>
	<title>Edit {data.pastor.name} - MOFI Admin</title>
</svelte:head>

<div class="flex items-center justify-between mb-6">
	<h1 class="text-2xl font-bold text-gray-900">Edit Pastor</h1>
	<a
		href="/admin/pastors"
	...
	<form method="POST" class="space-y-5" onsubmit={() => { submitting = true; }}>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<div class="space-y-1.5">
				...
				<input
					...
					id="title"
					name="title"
					value={form?.title ?? data.pastor.title}
					required
			...
			<div class="space-y-1.5">
				...
				<input
					...
					id="role"
					name="role"
					value={form?.role ?? data.pastor.role ?? ''}
		...
		<div class="space-y-1.5">
			...
			<textarea
				...
				rows="4"
				class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm"
			>{form?.bio ?? data.pastor.bio ?? ''}</textarea>
		</div>
		...
		<div class="space-y-1.5">
			...
			<input
				...
				id="ministryFocus"
				name="ministryFocus"
				value={form?.ministryFocus ?? data.pastor.ministryFocus ?? ''}
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<div class="space-y-1.5">
				...
				<input
					...
					id="phone"
					name="phone"
					value={form?.phone ?? data.pastor.phone ?? ''}
					class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm"
				/>
			</div>
			...
			<div class="space-y-1.5">
				...
				<input
					...
					id="email"
					name="email"
					value={form?.email ?? data.pastor.email ?? ''}
					class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm"
				/>
			</div>
		</div>
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
			<div class="space-y-1.5">
				...
				<input
					...
					id="sortOrder"
					name="sortOrder"
					value={form?.sortOrder ?? data.pastor.sortOrder}
					class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm"
				/>
			</div>
			...
			<div class="flex items-center gap-2 pt-6">
				<input
					...
					id="isActive"
					name="isActive"
					checked={form?.isActive ?? data.pastor.isActive}
		...
		<div class="flex items-center gap-3 pt-4">
			...
			>
				{submitting ? 'Saving...' : 'Update Pastor'}
			</button>
...
<!-- Church Assignments (read-only) -->
{#if data.churchAssignments.length > 0}
	<div class="mt-6 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
		<h2 class="text-lg font-semibold text-gray-900 mb-4">Church Assignments</h2>
		<p class="text-xs text-gray-400 mb-4">Assignments are managed from each church's edit page.</p>
		<div class="divide-y divide-gray-100">
			{#each data.churchAssignments as assignment}
				<div class="py-3 flex items-center justify-between">
					<div>
						<a
							href="/admin/churches/{assignment.churchId}"
							class="text-sm font-medium text-blue-600 hover:text-blue-800"
						>
							{assignment.churchName}
						</a>
						<span class="ml-2 text-xs text-gray-500">{assignment.role}</span>
						{#if assignment.isPrimary}
							<span class="ml-2 px-2 py-0.5 text-xs font-medium rounded-full bg-blue-100 text-blue-700">Primary</span>
						{/if}
						{#if assignment.since}
							<span class="ml-2 text-xs text-gray-400">since {assignment.since}</span>
						{/if}
					</div>
				</div>
			{/each}
