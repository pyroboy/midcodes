<script lang="ts">
	...
	import { allExpenses, getCategoryIcon } from '$lib/stores/expenses.svelte';
	...
	function opexForPeriod(p: Period) {
		...
		return Object.entries(grouped)
			.map(([label, amount]) => ({ label, icon: getCategoryIcon(label), amount }))
			.filter(e => e.amount > 0)
	...
	{#snippet kpis()}
		<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 flex-1">
			<KpiCard
				...
				changeLabel="revenue vs prev"
			/>
			<KpiCard label="Operating Expenses" value={formatPeso(totalOpex)} variant="danger" />
			<KpiCard label="Net Profit" value={formatPeso(netProfit)} variant={netProfit >= 0 ? 'success' : 'danger'} />
			<KpiCard label="Profit Margin" value="{netMarginPct.toFixed(1)}%" />
		</div>
	{/snippet}

	{#snippet chart()}
		<div class="flex-1 flex flex-col rounded-xl border border-border bg-white p-4">
			<p class="mb-3 text-xs font-bold uppercase tracking-wide text-gray-400">Revenue Trend — Last 14 Days</p>
			<div class="relative">
				<ReportBarChart data={chartData} yUnit="₱" height={200} showSecondary={false} primaryColor="#10B981" />
			</div>
		</div>
	{/snippet}
