<script lang="ts">
	import { session, LOCATIONS, getCurrentLocation, ELEVATED_ROLES } from '$lib/stores/session.svelte';
	...
	const BANNER_COLORS: Record<string, { bg: string; border: string; text: string; iconBg: string }> = {
		'qc':    { bg: 'bg-blue-50',     border: 'border-blue-200',    text: 'text-blue-900',    iconBg: 'bg-blue-100/50' },
		'mkti':  { bg: 'bg-violet-50',   border: 'border-violet-200',  text: 'text-violet-900',  iconBg: 'bg-violet-100/50' },
		'wh-qc': { bg: 'bg-amber-50',    border: 'border-amber-200',   text: 'text-amber-900',   iconBg: 'bg-amber-100/50' },
		'all':   { bg: 'bg-emerald-50',  border: 'border-emerald-200', text: 'text-emerald-900', iconBg: 'bg-emerald-100/50' },
	};

	const colors = $derived(BANNER_COLORS[session.locationId] ?? BANNER_COLORS['qc']);

	const accessLevel = $derived(
		session.role === 'admin' || session.role === 'owner' ? 'Full Inventory Control' :
...
<div class={cn('sticky top-0 z-30 w-full border-b px-4 py-3 sm:px-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-colors', colors.bg, colors.border)}>
	<div class="flex items-start sm:items-center gap-3">
		...
		<div>
			...
			<div class="mt-0.5 flex items-center gap-3 hidden sm:flex">
