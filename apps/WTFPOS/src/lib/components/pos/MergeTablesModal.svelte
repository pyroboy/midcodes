...
<ModalWrapper open={true} onclose={onclose} zIndex={60} ariaLabel="Merge tables" class="px-4">
	<!-- svelte-ignore a11y_no_static_element_interactions -->
	<div class="pos-card w-full max-w-[480px] min-h-[32rem] flex flex-col gap-4" use:pinpadKeyboard={{ onDigit: (d) => { if (step === 'pin') { pinError = false; if (pin.length < 4) { pin += d; playSound('click'); } } }, onBackspace: () => { if (step === 'pin') { pin = pin.slice(0, -1); pinError = false; } }, onClear: () => { if (step === 'pin') { pin = ''; pinError = false; } }, onSubmit: confirmMerge, canSubmit: () => step === 'pin' && pin.length === 4 }}>
		{#if step === 'select'}
			...
			<div class={cn("rounded-lg bg-red-50 border border-red-200 p-3 text-sm text-red-600", !mergeError && "invisible")}>
					⚠️ {mergeError || "\u00A0"}
				</div>
