...
export function calculateLeaseBalanceStatus(lease: Lease): LeaseBalanceStatus {
  ...
  const overdueBillings = lease.billings.filter(b => b.status === 'OVERDUE' || b.status === 'PENALIZED');
  ...
  const oldestOverdue = overdueBillings
    .sort((a, b) => new Date(a.due_date).getTime() - new Date(b.due_date).getTime())[0];
  const daysOverdue = oldestOverdue
    ? Math.floor((today.getTime() - new Date(oldestOverdue.due_date).getTime()) / (1000 * 60 * 60 * 24))
    : 0;
