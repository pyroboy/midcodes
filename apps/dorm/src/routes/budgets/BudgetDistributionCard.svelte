<script lang="ts">
	...
	let { plannedAmount = 0, allocatedAmount = 0, title = 'Budget Distribution' } = $props();
	...
	let remainingPercentage = $derived(
		plannedAmount > 0
			? Math.max(0, Math.min(Math.round((remainingAmount / plannedAmount) * 100), 100))
			: 0
	);
	...
	<CardContent class="pt-4">
		...
		<div class="space-y-4 mt-2">
			...
			{:else}
				<div class="bg-red-50 border border-red-200 rounded-md p-2 flex items-center gap-2">
					<span class="text-xs text-red-600"
						>Over budget by {formatCurrency(Math.abs(remainingAmount))}</span
					>
				</div>
			{/if}
			...
			<div class="pt-2 mt-2 border-t border-gray-200 text-xs text-gray-600">
				...
				<div class="mt-1">
					{#if remainingAmount >= 0}
						<span class="text-green-600 font-medium">{formatCurrency(remainingAmount)}</span> available
						to allocate
					{:else}
						<span class="text-red-600 font-medium">Exceeded</span> by {formatCurrency(
