<script lang="ts">
  ...
  import { DollarSign, CheckCircle, Clock, Plus, Edit, Trash2, RefreshCw, Info, CalendarRange } from 'lucide-svelte';
...
<Card class="bg-white shadow-md border border-gray-200 rounded-lg overflow-hidden mb-8">
  <div class="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-gray-200">
    ...
    <div class="p-4 flex-1">
      <div class="flex justify-between items-center mb-1">
        ...
        {#if data.statistics.totalPlannedBudget > 0}
          {@const allocatedPercentage = Math.min(Math.round((data.statistics.totalAllocatedBudget / data.statistics.totalPlannedBudget) * 100), 100)}
      ...
      {#if data.statistics.totalPlannedBudget > 0}
        ...
        <Progress 
          ...
          class={cn(
            "h-1.5 mb-1.5",
            allocatedPercentage < 85 ? "bg-green-100" : 
            allocatedPercentage >= 85 && allocatedPercentage < 100 ? "bg-yellow-100" : 
            "bg-red-100"
          )}
        />
    ...
    <div class="p-4 flex-1">
      ...
      <div class="flex gap-3 mb-1.5">
        ...
        <div class="flex items-center">
          <span class="h-5 w-5 rounded-full bg-yellow-100 mr-1.5 flex items-center justify-center">
  ...
  <form id="budget-form" method="POST" action="?/upsert" use:enhance>
    ...
    <input type="hidden" name="budget_items" value={JSON.stringify(sanitizeBudgetItems($form.budget_items || []))} />
