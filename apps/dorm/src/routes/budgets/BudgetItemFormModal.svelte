	...
	function handleSubmit() {
		if (validateForm()) {
			...
			const validatedItem: BudgetItem = {
				...formData,
				cost:
					typeof formData.cost === 'number'
						? formData.cost
						: parseFloat(formData.cost as unknown as string) || 0,
				quantity:
					typeof formData.quantity === 'number'
						? formData.quantity
						: parseFloat(formData.quantity as unknown as string) || 1
			};
		...
		>
			<Dialog.Header class="border-b pb-3">
				...
				<Dialog.Description class="text-gray-600">
					{editMode ? 'Update the details of this budget item.' : 'Add a new item to your budget.'}
				</Dialog.Description>
			</Dialog.Header>
			...
			<Dialog.Footer class="flex justify-end gap-2 pt-4 border-t mt-2">
				<Button variant="outline" onclick={handleClose} class="border-gray-300 text-gray-700">
					Cancel
				</Button>
