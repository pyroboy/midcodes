...
<Card class="bg-white shadow-sm border border-gray-100 rounded-lg overflow-hidden hover:shadow-md transition-all duration-200">
  <!-- Single Row Header -->
  ...
  <CardContent class="px-4 py-3">
    ...
    <div class="grid grid-cols-3 gap-2 mb-3">
      <div class="bg-gray-50 p-2 rounded-md border border-gray-100">
        ...
        <div class="text-sm font-bold text-gray-800">{safeFormatCurrency(budget.planned_amount)}</div>
      </div>
      ...
      <div class="bg-gray-50 p-2 rounded-md border border-gray-100">
        ...
        <div class="text-sm font-bold" class:text-green-600={budget.remainingAmount >= 0} class:text-red-600={budget.remainingAmount < 0}>
    ...
    <div class="mb-3">
      <Progress 
        ...
        class={`h-1.5 ${getProgressColor(budget)}`} 
      />
    ...
    <div class="mt-2">
      ...
      {#if !budget.budget_items?.length}
        <div class="text-center py-2 px-3 bg-gray-50 rounded-md border border-dashed border-gray-200">
        ...
        <div class="overflow-hidden rounded-md border border-gray-200 text-xs">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th scope="col" class="px-3 py-1.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Item</th>
                <th scope="col" class="px-3 py-1.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Type</th>
                <th scope="col" class="px-3 py-1.5 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Cost</th>
                <th scope="col" class="px-3 py-1.5 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Qty</th>
                <th scope="col" class="px-3 py-1.5 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Total</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              {#each budget.budget_items as item, i}
                ...
                <tr class="hover:bg-gray-50">
                  ...
                  <td class="px-3 py-1.5 text-xs text-gray-500">
                    {#if item.type}
                      <span class="inline-block px-1.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                        {item.type}
                      </span>
                    {:else}
                  ...
                  <td class="px-3 py-1.5 text-xs text-gray-500 text-right">{safeFormatCurrency(cost)}</td>
              ...
              <tr class="bg-gray-50 font-medium">
                <td colspan="4" class="px-3 py-1.5 text-xs text-gray-700 text-right">Total Allocated:</td>
  ...
  <CardFooter class="px-4 py-2 bg-gray-50 border-t text-xxs text-gray-500 flex justify-between items-center">
