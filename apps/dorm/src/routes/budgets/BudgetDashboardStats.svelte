<script lang="ts">
	...
	let allocatedPercentage = $derived(
		statistics.totalPlannedBudget > 0
			? Math.min(
					Math.round((statistics.totalAllocatedBudget / statistics.totalPlannedBudget) * 100),
					100
				)
			: 0
	);
	let completionPercentage = $derived(
		totalProjects > 0 ? Math.round((statistics.completedProjects / totalProjects) * 100) : 0
	);
