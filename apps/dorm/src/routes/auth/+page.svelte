	...
	const handleSubmit: SubmitFunction = () => {
		...
		return async ({ result, update }) => {
			if (result.type === 'redirect') {
				await Promise.all([
					invalidateAll(),
					goto('/') // Add this line to redirect to home
				]);
				return;
			}
			await update();
		...
		<Card class="w-full p-6 shadow-lg">
			<CardHeader class="space-y-4 text-center">
				...
				<CardTitle class="text-3xl font-bold tracking-tight"
					>Welcome to March of Faith, Inc.</CardTitle
				>
			...
			<CardContent class="pt-6">
				<Tabs value="signin" class="w-full">
					{#if !disableSignUp}
						<TabsList
							class="grid w-full {disableSignUp
								? 'grid-cols-1'
								: 'grid-cols-2'} gap-4 rounded-lg p-1"
						>
					...
					<TabsContent value="signin">
						<form method="POST" action="?/signin" use:enhance={handleSubmit} class="space-y-6">
							<div class="space-y-4">
								<div class="space-y-2">
									<Input
										id="signin-email"
