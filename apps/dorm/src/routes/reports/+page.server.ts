...
export const load: PageServerLoad = async ({ locals: { supabase, safeGetSession }, url }) => {
  ...
  try {
    ...
    if (leaseTenantsError) throw new Error(`Error fetching lease tenants: ${leaseTenantsError.message}`);
    ...
    for (const [floorId, billings] of Object.entries(floorBillingsMap)) {
      ...
      floorDataMap[floorId].collectible = billings.reduce((sum, billing) => {
        if (billing.type === 'RENT' && (billing.status === 'PENDING' || billing.status === 'PARTIAL')) {
    ...
    for (const [floorId, billings] of Object.entries(floorBillingsMap)) {
      if (payments && payments.length > 0) {
        for (const payment of payments) {
          ...
          if (paymentDate >= startDate && paymentDate <= new Date(endDate.getTime() + 86400000 - 1)) {
            const billingIds = payment.billing_ids || [];
            
            // Check if any billing ID in this payment belongs to this floor
            const appliesToFloor = billings.some(billing => 
    ...
    const grossIncome = Object.values(floorDataMap).reduce((sum, floor: any) => sum + Number(floor.income), 0);
    
    // Calculate collectibles (all unpaid balances) for the current month only
    const collectibles = Object.values(floorDataMap).reduce((sum, floor: any) => sum + Number(floor.collectible), 0);
    ...
    const totalOperationalExpenses = operationalExpenses.reduce((sum, expense) => sum + Number(expense.amount), 0);
    const totalCapitalExpenses = capitalExpenses.reduce((sum, expense) => sum + Number(expense.amount), 0);
