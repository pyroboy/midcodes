<script lang="ts">
  ...
  import type { FloorData, FloorDataMap, Expense, MonthData, Property, Month, ProfitSharingCalculation } from './types';
  ...
  function calculateProfitSharing(grossIncome: number, expenses: Expense[]): { fortyShare: number; sixtyShare: number } {
...
<div class="max-w-6xl mx-auto space-y-6">
  <div class="bg-white rounded-xl shadow-md p-6 border-t-4 border-blue-500">
    <div class="text-center space-y-4 mb-6">
      ...
      <div class="flex items-center justify-center gap-2 my-2">
        <span class="bg-blue-50 text-blue-800 text-lg font-medium px-4 py-1 rounded-lg border border-blue-200">
  ...
  {#if !propertyId}
    <div class="bg-white p-8 rounded-xl shadow-md text-center">
      ...
      <p class="text-gray-600 max-w-md mx-auto">Select a property from the dropdown above to view the financial report for {formatMonth(month)} {year}.</p>
    </div>
  {:else if !reportData}
  ...
  {:else}
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <!-- Summary Card -->
      <div class="bg-white rounded-xl overflow-hidden shadow-md md:col-span-2 border border-gray-200 hover:shadow-lg transition-shadow duration-200">
        <div class="p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="bg-green-50 rounded-lg p-4 border border-green-100">
              ...
              <p class="text-2xl font-bold text-green-800">{formatCurrency(reportData.totals.grossIncome)}</p>
            </div>
            ...
            <div class="bg-red-50 rounded-lg p-4 border border-red-100">
              ...
              <p class="text-2xl font-bold text-red-800">{formatCurrency(calculateOperationalExpenses(reportData.expenses) + calculateCapitalExpenses(reportData.expenses))}</p>
            </div>
            ...
            <div class="bg-indigo-50 rounded-lg p-4 border border-indigo-100">
              ...
              <p class="text-2xl font-bold text-indigo-800">{formatCurrency(reportData.totals.grossIncome - (calculateOperationalExpenses(reportData.expenses) + calculateCapitalExpenses(reportData.expenses)))}</p>
            </div>
            
            <div class="bg-amber-50 rounded-lg p-4 border border-amber-100">
              <div class="flex items-center mb-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-amber-600 mr-2" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div>
                <p class="text-2xl font-bold text-amber-800">{formatCurrency(reportData.totals.collectibles)}</p>
                {#if reportData.totals.lastCollectionDate}
                  <p class="text-xs text-amber-600 mt-1">Last collection: {formatDate(reportData.totals.lastCollectionDate)}</p>
                {/if}
              </div>
            </div>
          </div>
      ...
      <div class="bg-white rounded-xl overflow-hidden shadow-md">
        ...
        <div class="p-5">
              ...
              <div class="p-4 space-y-4">
                ...
                <div class="rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-200">
                  ...
                  <div class="p-4 flex items-center">
                    ...
                    <div class="flex-1">
                      <div class="text-2xl font-bold text-gray-800">{formatCurrency(floor.income)}</div>
                    </div>
                    ...
                    <div class="relative group">
                      <div class="w-16 h-16 relative">
                        ...
                        <svg class="w-full h-full" viewBox="0 0 36 36">
                          <circle cx="18" cy="18" r="15.9" fill="none" stroke-width="3" stroke="#f3f4f6" />
                          ...
                          <!-- Percentage text -->
                          <text x="18" y="18" text-anchor="middle" dominant-baseline="middle" font-size="8" font-weight="bold" fill="#4b5563">
                      ...
                      <!-- Tooltip on hover -->
                      <div class="absolute right-0 z-50 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-white border border-gray-200 rounded-lg shadow-xl p-3 w-64 top-full mt-1">
                        <div class="text-sm mb-2 font-semibold text-gray-700 flex items-center gap-1">
                          <Info class="h-4 w-4" />
                        ...
                        <div class="space-y-2">
                          <div class="flex justify-between items-center">
                            ...
                            <span class="text-xs font-medium text-green-600">{formatCurrency(floor.collected)}</span>
                          </div>
                          <div class="flex justify-between items-center">
                            <span class="text-xs text-gray-600">Collectible (Unpaid)</span>
                            <span class="text-xs font-medium text-amber-600">{formatCurrency(floor.collectible)}</span>
                          </div>
                          {#if floor.lastCollectionDate}
                          <div class="flex justify-between items-center">
                            <span class="text-xs text-gray-600">Last Collection</span>
                            <span class="text-xs font-medium text-gray-600">{formatDate(floor.lastCollectionDate)}</span>
                          </div>
                          {/if}
                        </div>
                      </div>
                ...
                <!-- Total -->
                <div class="rounded-lg border border-gray-200 overflow-hidden col-span-1 md:col-span-2 xl:col-span-3">
                  ...
                  <div class="p-4">
                    <div class="flex items-center justify-between">
                      <div>
                        <div class="text-2xl font-bold text-gray-800">{formatCurrency(reportData.totals.grossIncome)}</div>
                      </div>
                      <div class="flex gap-6 text-sm">
                        <div>
                          <div class="text-gray-500 mb-1">Collected</div>
                          <div class="font-semibold text-green-600">{formatCurrency(reportData.totals.collected)}</div>
                        </div>
                        <div>
                          <div class="text-gray-500 mb-1">Collectible</div>
                          <div class="font-semibold text-amber-600">{formatCurrency(reportData.totals.collectibles)}</div>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <div class="bg-white rounded-xl overflow-hidden shadow-md">
        ...
        <div class="p-5">
          {#if reportData}
            {@const profitSharing = calculateProfitSharing(reportData.totals.grossIncome, reportData.expenses)}
            <div class="space-y-4">
              <div class="flex justify-between items-center py-2 px-3 bg-gray-50 rounded-lg">
                <span class="font-medium text-gray-700">Gross Income</span>
                <span class="font-semibold text-green-600">{formatCurrency(reportData.totals.grossIncome)}</span>
              </div>
              <div class="flex justify-between items-center py-2 px-3 bg-gray-50 rounded-lg">
                <span class="font-medium text-gray-700">Operational Expenses</span>
                <span class="font-semibold text-red-600">- {formatCurrency(calculateOperationalExpenses(reportData.expenses))}</span>
              </div>
              <div class="flex justify-between items-center py-2 px-3 bg-gray-50 rounded-lg">
                <span class="font-medium text-gray-700">Capital Expenses (60% share only)</span>
                <span class="font-semibold text-red-600">- {formatCurrency(calculateCapitalExpenses(reportData.expenses))}</span>
              </div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                <div class="flex justify-between items-center py-3 px-4 bg-indigo-100 rounded-lg font-bold">
                  <span class="text-indigo-800">40% Share</span>
                  <span class="text-indigo-800">
                    {formatCurrency(profitSharing.fortyShare)}
                  </span>
                </div>
                <div class="flex justify-between items-center py-3 px-4 bg-blue-100 rounded-lg font-bold">
              ...
              <div class="mb-4 px-4 py-3 bg-blue-50 rounded-lg text-sm text-gray-600">
                <p class="flex items-center">
                  ...
                  <span>Profit sharing is calculated based on collected amounts with operational expenses deducted from both shares and capital expenses deducted from 60% share only. Collectibles will be included when collected.</span>
                </p>
              </div>
            </div>
          {/if}
            ...
            <div class="border-r-0 md:border-r border-gray-200 pr-0 md:pr-6">
              ...
              <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    {:else}
                      {#each getExpensesByType(reportData.expenses, 'OPERATIONAL') as expense}
                        <tr>
                          ...
                          <td class="px-4 py-2 text-sm font-medium">{formatCurrency(expense.amount)}</td>
                        </tr>
                      {/each}
                    {/if}
                  </tbody>
                  <tfoot>
                    <tr class="bg-gray-50">
                      <td colspan="2" class="px-4 py-3 text-right font-bold text-gray-800">Total</td>
                      <td class="px-4 py-3 font-bold">{formatCurrency(calculateTotalByType(reportData.expenses, 'OPERATIONAL'))}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
              ...
              <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                  ...
                  <tfoot>
                    <tr class="bg-gray-50">
                      <td colspan="2" class="px-4 py-3 text-right font-bold text-gray-800">Total</td>
                      <td class="px-4 py-3 font-bold">{formatCurrency(calculateTotalByType(reportData.expenses, 'CAPITAL'))}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
