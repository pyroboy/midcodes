<script lang="ts">
	...
	function getConsumptionByType(): ChartDataset {
		...
		const validReadings = props.readings.filter(
			(r: Reading) => r.consumption !== null && r.consumption !== undefined
		);
	...
	function getConsumptionTrend(): ChartDataset {
		...
		return {
			...
			colors: Array(chartData.length).fill(
				props.selectedType ? getUtilityColor(props.selectedType) : '#3B82F6'
			)
		};
	}
	...
	let maxTypeValue = $derived(
		consumptionByType.values.length > 0 ? Math.max(...consumptionByType.values) : 0
	);
	let maxTrendValue = $derived(
		consumptionTrend.values.length > 0 ? Math.max(...consumptionTrend.values) : 0
	);
		...
		<Card.Content>
			...
			{:else}
				<div class="relative h-64">
					<!-- Y-axis labels -->
					<div
						class="absolute left-0 top-0 h-full flex flex-col justify-between text-xs text-gray-500 py-2"
					>
						...
						{#each consumptionByType.labels as label, i}
							<div class="flex flex-col items-center w-full max-w-16">
								<div
									...
									style="height: {getChartBarHeight(
										consumptionByType.values[i],
										maxTypeValue,
										200
									)}px; background-color: {consumptionByType.colors[i]};"
								></div>
						...
						{#each consumptionTrend.labels as label, i}
							<div class="flex flex-col items-center w-full max-w-16">
								<div
									class="w-12 rounded-t-md transition-all duration-500"
									style="height: {getChartBarHeight(
										consumptionTrend.values[i],
										maxTrendValue,
										200
									)}px; background-color: {consumptionTrend.colors[i]};"
								></div>
								<span class="mt-2 text-xs font-medium">{label}</span>
