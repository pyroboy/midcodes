...
export const billingTypeEnum = z.enum(['RENT', 'UTILITY', 'MISC', 'PENALTY']);

export type BillingType = z.infer<typeof billingTypeEnum>;

export const utilityTypeEnum = z
...
export const penaltyBillingSchema = z.object({
	...
	billing_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: 'Invalid date format' }),
