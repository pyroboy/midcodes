      ...
      {#if penalties && penalties.length > 0}
        {#each penalties as penalty}
          ...
          >
              ...
              {#if penalty.lease?.lease_tenants && penalty.lease.lease_tenants.length > 0}
                <div class="flex flex-col space-y-1">
                  {#each penalty.lease.lease_tenants as leaseTenant, i}
                    ...
                    {:else if i === 2}
                      <span class="text-sm text-gray-500">+{penalty.lease.lease_tenants.length - 2} more</span>
                    {/if}
                  {/each}
                </div>
              {:else}
            ...
            <TableCell>
              {#if penalty.penalty_amount > 0}
                <span class="font-medium text-red-600">{formatCurrency(penalty.penalty_amount)}</span>
              {:else}
