<script lang="ts">
  ...
  import { AlertTriangle, DollarSign, Clock, Search, LayoutGrid, List, Settings, Filter, RefreshCw } from 'lucide-svelte';
  ...
  let overdueCount = $derived(
    penaltyBillings.filter(billing => new Date(billing.due_date) < new Date() && billing.balance > 0).length
  );
    ...
    <div class="max-w-7xl mx-auto px-4 py-6">
      <div class="flex flex-col gap-4">
        ...
        <nav class="flex items-center text-sm text-muted-foreground">
          <a href="/" class="hover:text-foreground transition-colors" data-sveltekit-preload-data="hover">
            Dashboard
          </a>
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" onclick={() => viewMode = viewMode === 'list' ? 'card' : 'list'}>
              {#if viewMode === 'list'}
  ...
  <div class="max-w-7xl mx-auto px-4 py-6 space-y-6">
    ...
    {:else}
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <!-- Total Penalties -->
        <Card 
          class="cursor-pointer transition-all hover:shadow-md {activeFilter === 'all' ? 'ring-2 ring-primary' : ''}"
        ...
        <!-- Overdue Billings -->
        <Card 
          class="cursor-pointer transition-all hover:shadow-md {activeFilter === 'overdue' ? 'ring-2 ring-destructive' : ''}"
        ...
        <!-- Pending Payments -->
        <Card 
          class="cursor-pointer transition-all hover:shadow-md {activeFilter === 'pending' ? 'ring-2 ring-orange-500' : ''}"
        ...
        <!-- Penalized Billings -->
        <Card 
          class="cursor-pointer transition-all hover:shadow-md {activeFilter === 'penalized' ? 'ring-2 ring-yellow-500' : ''}"
            ...
            <div class="flex items-center justify-between">
              <div>
                <CardTitle class="text-xl">
                  {activeFilter === 'all' ? 'All Penalties' : 
                   activeFilter === 'pending' ? 'Pending Penalties' :
                   activeFilter === 'overdue' ? 'Overdue Penalties' : 
                   activeFilter === 'penalized' ? 'Penalized Billings' : 'Penalties'}
                </CardTitle>
                <CardDescription>
                ...
                {#if filteredBillings.length === 0}
                  <div class="text-center py-12">
                    ...
                    <p class="text-muted-foreground">
                      {activeFilter === 'all' ? 'No penalty records available.' :
                ...
                {:else}
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {#each filteredBillings as penalty (penalty.id)}
                      <Card class="cursor-pointer hover:shadow-md transition-all" onclick={() => handlePenaltyClick(penalty)}>
                        <CardContent class="p-4">
                          <div class="space-y-3">
                            <div class="flex items-start justify-between">
                              ...
                              <Badge 
                                variant={penalty.status === 'PENDING' ? 'default' : 
                                        penalty.status === 'OVERDUE' ? 'destructive' : 
                                        penalty.status === 'PAID' ? 'secondary' : 'outline'}
                                class="text-xs"
                              >
                                {penalty.status}
                              </Badge>
