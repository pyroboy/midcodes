<script lang="ts">
  ...
  import { Download, Edit, Eye, FileText, Filter, Plus, RefreshCw, Trash2, Receipt } from 'lucide-svelte';
...
<div class="space-y-6">
  ...
  <div class="flex flex-col sm:flex-row justify-between gap-4">
    ...
    <div class="flex gap-2">
      <Button variant="outline" onclick={() => showFilters = !showFilters} class="flex items-center gap-1">
      ...
      <Button variant="ghost" onclick={handleRefresh} class="flex items-center gap-1" title="Refresh">
        ...
        <table class="w-full">
          <thead>
            <tr class="bg-gray-50 border-b">
              ...
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Lease</th>
          ...
          <tbody class="divide-y divide-gray-200">
            {#each getFilteredTransactions() as transaction (transaction.id)}
              <tr class="hover:bg-gray-50">
                <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{transaction.id}</td>
                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{formatDate(transaction.paid_at)}</td>
                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 font-medium">{formatCurrency(transaction.amount)}</td>
                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{transaction.paid_by}</td>
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{transaction.reference_number || 'N/A'}</td>
                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                  {#if transaction.receipt_url}
                    <a href={transaction.receipt_url} target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:underline flex items-center">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                  <div class="flex gap-2">
                    ...
                    <Button variant="ghost" size="icon" onclick={() => handleEditTransaction(transaction)} title="Edit">
                      <Edit class="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onclick={() => handleDeleteTransaction(transaction.id || 0)} title="Delete">
              ...
              <tr>
                <td colspan="9" class="px-4 py-12 text-center text-gray-500">
                  <div class="flex flex-col items-center justify-center gap-2">
                    ...
                    <p class="text-sm text-gray-400">Try adjusting your filters or add a new transaction</p>
                  </div>
                </td>
              </tr>
            {/if}
