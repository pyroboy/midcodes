<script lang="ts">
	...
	const filterMeters = () => {
		if (!metersData || !Array.isArray(metersData)) return [];
		...
		return filtered.sort((a: ExtendedMeterFormData, b: ExtendedMeterFormData) => {
			...
			switch (sortBy) {
				...
				case 'reading':
					return order * ((a.latest_reading?.value || 0) - (b.latest_reading?.value || 0));
				default:
					return 0;
			}
		});
	...
	const groupMeters = (): MeterGroup[] => {
		...
		filteredMeters.forEach((meter) => {
			...
			if (!grouped[propId]) {
				grouped[propId] = { id: propertyId, name: propertyName, meters: [] };
			}
	...
	const { form, enhance, errors, constraints, submitting, reset, message } = superForm((() => meterForm)(), {
		id: 'locations-meter-form',
