...
>
	...
	<div class="space-y-2">
		...
		>
			<Select.Trigger class="w-full" data-error={!!$errors.floor_id} {...$constraints.floor_id}>
				{triggerFloorContent}
			</Select.Trigger>
			<Select.Content>
	...
	<div class="space-y-2">
		...
		<Select.Root type="single" name="type" bind:value={typeValue.value}>
	...
	<div class="space-y-2">
		...
		<Select.Root type="single" name="rental_unit_status" bind:value={statusValue.value}>
			<Select.Trigger
				class="w-full"
				...
				{#each amenities.value as amenity, i}
					<div class="flex items-center gap-1 bg-secondary/50 p-1 rounded">
						...
						<button
							...
							onclick={() => removeAmenity(i)}>×</button
						>
					</div>
				{/each}
			</div>
