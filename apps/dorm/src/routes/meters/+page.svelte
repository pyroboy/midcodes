					...
					<div>
						...
						<div class="space-y-2">
							{#each group.meters as meter}
								<Card.Root
									class={`${selectedMeter?.id === meter.id ? 'border-2 border-blue-500' : ''} hover:bg-gray-50 transition-colors`}
								>
									<button
										type="button"
	...
	<div class="w-full lg:w-1/3 lg:sticky lg:top-0 lg:h-screen lg:overflow-auto">
		<div class="bg-white p-4 rounded-lg border">
			<div class="flex justify-between items-center mb-3">
				...
				<span class="text-xs text-gray-500">
					Edit Mode: {editMode ? 'YES' : 'NO'} | Selected Meter ID: {selectedMeter?.id || 'none'}
				</span>
			</div>
