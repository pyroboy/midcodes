<script lang="ts">
	...
	function getStatusSymbolColor(payment: any, type: 'rent' | 'utilities' | 'penalty'): string {
		...
		const amountField =
			type === 'rent' ? 'rentAmount' : type === 'utilities' ? 'utilitiesAmount' : 'penaltyAmount';
	...
	function getOverallStatus(payment: any): PaymentStatus {
		if (
			payment.rent === 'OVERDUE' ||
			payment.utilities === 'OVERDUE' ||
			payment.penalty === 'OVERDUE'
		) {
			return 'OVERDUE';
		} else if (
			payment.rent === 'PENDING' ||
			payment.utilities === 'PENDING' ||
			payment.penalty === 'PENDING'
		) {
			return 'PENDING';
		} else if (
			payment.rent === 'PARTIAL' ||
			payment.utilities === 'PARTIAL' ||
			payment.penalty === 'PARTIAL'
		) {
			return 'PARTIAL';
		} else {
			return 'PAID';
					...
					{#each unit.tenantRecords as tenantRecord, tenantIndex}
						<Table.Row>
							...
							<Table.Cell class="align-top">
								<Tooltip.Root>
									<Tooltip.Trigger>
										<div class="font-medium cursor-help underline decoration-dotted">
											{tenantRecord.tenantName}
										</div>
									</Tooltip.Trigger>
									<Tooltip.Content>
							...
							{#each tenantRecord.monthlyPayments as payment}
								<Table.Cell class="text-center py-2 px-1">
									<Tooltip.Root>
										<Tooltip.Trigger>
											<div
												class={`inline-flex items-center border rounded px-3 py-2 ${getCellBackgroundColor(getOverallStatus(payment))}`}
											>
												...
												<Card.Content class="p-3 space-y-3">
													...
													<div class="bg-gray-50 p-2 rounded text-xs mb-2 space-y-1">
														...
														<div class="flex flex-col gap-1">
															<span>
																<span class="text-green-600 font-medium">Filled symbol (●,■)</span>:
																Paid or Partial
															</span>
															<span>
																<span class="text-orange-500 font-medium">Empty symbol (○,□)</span>:
																Pending or Overdue
															</span>
															<span>
																<span class="text-gray-400 font-medium">Light symbol</span>: No
																amount/billing
															</span>
														</div>
													</div>
													...
													<div class="space-y-1">
														<div class="flex justify-between items-center">
															<span class="text-xs font-medium flex items-center">
																<span
																	class={`inline-block mr-1 text-sm ${getStatusSymbolColor(payment, 'rent')}`}
																>
																	{getPaymentSymbol(payment, 'rent')}
																</span>
																Rent:
															...
															<span
																class={`text-xs px-2 rounded ${(payment.rentAmount || 0) === 0 ? 'bg-gray-50 text-gray-500' : getCellBackgroundColor(payment.rent)}`}
															>
													...
													<div class="space-y-1">
														<div class="flex justify-between items-center">
															<span class="text-xs font-medium flex items-center">
																<span
																	class={`inline-block mr-1 text-sm ${getStatusSymbolColor(payment, 'utilities')}`}
																>
																	{getPaymentSymbol(payment, 'utilities')}
																</span>
																Utilities:
															...
															<span
																class={`text-xs px-2 rounded ${(payment.utilitiesAmount || 0) === 0 ? 'bg-gray-50 text-gray-500' : getCellBackgroundColor(payment.utilities)}`}
															>
													...
													<div class="space-y-1">
														<div class="flex justify-between items-center">
															<span class="text-xs font-medium flex items-center">
																<span
																	class={`inline-block mr-1 text-sm ${getStatusSymbolColor(payment, 'penalty')}`}
																>
																	{getPaymentSymbol(payment, 'penalty')}
																</span>
																Penalty:
															...
															<span
																class={`text-xs px-2 rounded ${(payment.penaltyAmount || 0) === 0 ? 'bg-gray-50 text-gray-500' : getCellBackgroundColor(payment.penalty)}`}
															>
													...
													<div class="pt-2 mt-1 border-t border-gray-200">
														...
														<div class="text-xs flex justify-between font-medium">
															...
															<span
																>{formatCurrency(
																	(payment.rentPaidAmount || 0) +
																		(payment.utilitiesPaidAmount || 0) +
																		(payment.penaltyPaidAmount || 0)
																)}</span
															>
														</div>
													</div>
