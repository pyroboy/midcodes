<script lang="ts">
  ...
  const initializeRents = (year: number, existingBillings: any[]) => {
    const billingsMap = new Map(existingBillings.map(b => [new Date(b.billing_date).getMonth() + 1, b]));
  ...
  const handleSaveChanges = async () => {
    ...
    monthlyRents.forEach((rent, index) => {
      if (rent.isActive) {
        ...
        if (selectedDate.getMonth() + 1 !== expectedMonth || selectedDate.getFullYear() !== expectedYear) {
          console.log(`⚠️ Due date ${rent.dueDate} is not in month ${expectedMonth}/${expectedYear}`);
          validationErrors.push(`Month ${rent.month}: Due date must be in ${getMonthName(expectedMonth)} ${expectedYear}`);
        }
      }
    });
    ...
    try {
      ...
      console.log('📤 Sending data to server:', {
        ...
        dueDates: monthlyRents.filter(r => r.isActive).map(r => ({ month: r.month, dueDate: r.dueDate }))
      });
  ...
  const changeYear = (offset: number) => {
    if (hasUnsavedChanges) {
      const confirmed = confirm('You have unsaved changes. Are you sure you want to change the year?');
      if (!confirmed) return;
    }
    selectedYear += offset;
    fetchBillingsForYear(selectedYear);
        ...
        <div class="text-sm text-muted-foreground mr-auto">
          {#if monthlyRents.filter(r => r.isActive).length > 0}
            <span class="text-green-600">
              {monthlyRents.filter(r => r.isActive).length} month{monthlyRents.filter(r => r.isActive).length !== 1 ? 's' : ''} will be billed
            </span>
          {/if}
          {#if monthlyRents.filter(r => !r.isActive && r.billingId).length > 0}
            <span class="text-red-600 ml-2">
              {monthlyRents.filter(r => !r.isActive && r.billingId).length} billing{monthlyRents.filter(r => !r.isActive && r.billingId).length !== 1 ? 's' : ''} will be removed
            </span>
          {/if}
        </div>
      {/if}
