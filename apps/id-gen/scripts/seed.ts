...
import { rolePermissions, templateSizePresets, orgSettings, organizations } from '../src/lib/server/schema';
...
const PERMISSIONS = {
    super_admin: [
        'payments.read', 'payments.update', 'payments.delete',
        'payment_schedules.manage', 'payment_schedules.read',
        'penalties.configure',
        'meters.create', 'meters.read', 'meters.update', 'meters.delete',
        'readings.create', 'readings.read', 'readings.update', 'readings.delete',
        'events.create', 'events.read', 'events.update', 'events.delete',
        'attendees.create', 'attendees.read', 'attendees.update', 'attendees.delete', 'attendees.check_qr',
        ...
        'bookings.read', 'bookings.update', 'bookings.delete'
    ],
    org_admin: [
        ...
        'attendees.create', 'attendees.read', 'attendees.update', 'attendees.check_qr',
    ...
    id_gen_user: [
        'templates.read',
        'idcards.create', 'idcards.read', 'idcards.update',
        'profiles.read', 'profiles.update'
    ],
    user: [
        'events.read',
...
async function seed() {
    ...
    try {
        ...
        for (const [role, permissions] of Object.entries(PERMISSIONS)) {
            for (const permission of permissions) {
                const check = await db.query.rolePermissions.findFirst({
                    where: (rp, { and, eq }) => and(eq(rp.role, role as any), eq(rp.permission, permission as any))
                });
        ...
        for (const preset of SIZE_PRESETS) {
            await db.insert(templateSizePresets).values(preset as any)
                .onConflictDoUpdate({
