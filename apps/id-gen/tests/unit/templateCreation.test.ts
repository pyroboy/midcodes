...
describe('Image Upload Schema Validation', () => {
    ...
    it('should handle uppercase file extensions', () => {
        ...
        const result = imageUploadSchema.safeParse({ file: upperExtFile, side: 'front', expectedWidth: 100, expectedHeight: 100 });
        expect(result.success).toBe(true);
    });

    it('should validate side as back', () => {
        const backFile = new File(['test'], 'back.png', { type: 'image/png' });
        const result = imageUploadSchema.safeParse({ file: backFile, side: 'back', expectedWidth: 100, expectedHeight: 100 });
        expect(result.success).toBe(true);
    });

    it('should validate with minimum dimensions', () => {
        const minDimFile = new File(['test'], 'min.png', { type: 'image/png' });
        const result = imageUploadSchema.safeParse({ file: minDimFile, side: 'front', expectedWidth: 1, expectedHeight: 1 });
        expect(result.success).toBe(true);
    });
    ...
    it('should handle a very long filename', () => {
        ...
        const result = imageUploadSchema.safeParse({ file: longFile, side: 'front', expectedWidth: 100, expectedHeight: 100 });
        expect(result.success).toBe(true);
    });
});
