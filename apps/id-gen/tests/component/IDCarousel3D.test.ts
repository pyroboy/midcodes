...
vi.mock('$lib/utils/supabase', () => ({
    getSupabaseStorageUrl: (path: string, bucket: string) => `https://supabase.co/storage/v1/${bucket}/${path}`
}));
 ...
 */
function transformTexture(texture: THREE.Texture, imageWidth: number, imageHeight: number): THREE.Texture {
 ...
 */
function calculateDragNavigation(dragOffset: number, currentIndex: number, cardsLength: number): number {
...
describe('IDCarousel3D Component Logic', () => {
    ...
    describe('Texture Transformation', () => {
        ...
        function createMockTexture(): MockTexture {
            const repeat = { x: 1, y: 1, set: function (x: number, y: number) { this.x = x; this.y = y; } };
            const offset = { x: 0, y: 0, set: function (x: number, y: number) { this.x = x; this.y = y; } };
            return {
                colorSpace: '',
                wrapS: 0,
                wrapT: 0,
        ...
        function transformMockTexture(texture: MockTexture, imageWidth: number, imageHeight: number): MockTexture {
            texture.colorSpace = THREE.SRGBColorSpace;
            texture.wrapS = THREE.ClampToEdgeWrapping;
            texture.wrapT = THREE.ClampToEdgeWrapping;
