<script lang="ts">
    ...
    let { 
        ...
        backgroundPosition = $bindable({ x: 0, y: 0, scale: 1 } as { x: number; y: number; scale: number }),
    ...
    let elementStyle = $derived((element: TemplateElement) => {
        ...
        if (element.variableName === 'name' || element.variableName === 'photo') {
            console.log(`📍 Element "${element.variableName}" positioning:`, {
                ...
                adjustedStyle, coordSystemScale: currentCoordSystem.scale
            });
        }
    ...
    let textStyle = $derived((element: TemplateElement) => {
        ...
        return {
            ...
            'letter-spacing': element.letterSpacing ? `${Math.round(element.letterSpacing * scale)}px` : 'normal',
...
<div class="template-section">
    ...
    <div class="template-layout">
        <div class="preview-container" 
             style="max-width: {baseDimensions().width || 0}px; aspect-ratio: {baseDimensions().width || 1}/{baseDimensions().height || 1};">
            <div 
                ...
                style="background-image: {preview ? `url('${preview}')` : 'none'}; 
                       ...
                       background-position: {preview ? `${50 + backgroundPosition.x}% ${50 + backgroundPosition.y}%` : 'center'};
            ...
            >
                    ...
                    <label class="placeholder-design clickable-container">
                        ...
                        <div class="placeholder-content">
                            ...
                            <p class="text-sm text-muted-foreground mb-4">
                                Required size: {baseDimensions().actualWidth || 0}×{baseDimensions().actualHeight || 0} pixels
                                {#if cardSize}
                                    <br>
                                    <span class="text-xs">
                ...
                {/each}
                <Button variant="destructive" size="icon" class="remove-image" onclick={() => onRemoveImage(side)}>
                    <X class="w-4 h-4" />
                </Button>
            {/if}
        </div>
