<script lang="ts">
  ...
  const position = tweened<[number, number, number]>([0, 0, 0], { duration: 1500, easing: cubicOut });
  ...
  let materials = $derived([
    ...
    edgeMaterial, // top
    edgeMaterial, // bottom
    frontTexture ? new THREE.MeshStandardMaterial({ map: frontTexture, roughness: 0.4, metalness: 0.1 }) : edgeMaterial, // front
    backTexture ? new THREE.MeshStandardMaterial({ map: backTexture, roughness: 0.4, metalness: 0.1 }) : edgeMaterial  // back
  ]);
