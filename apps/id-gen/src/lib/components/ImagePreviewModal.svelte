<script lang="ts">
	...
	$effect(() => {
		...
		if (templateDimensions && browser) {
			console.log(
				'[ImagePreviewModal] Creating custom geometry for dimensions:',
				templateDimensions
			);
	...
	let debugLogs = $state<
		Array<{ timestamp: number; message: string; type: 'info' | 'error' | 'success' }>
	>([]);
						...
						<Canvas>
							<T.Scene>
								...
								<T.Group rotation.y={rotationY} scale={responsiveSettings().modelScale}>
									...
									{#await Promise.all( [frontImageUrl ? useTexture(frontImageUrl) : Promise.resolve(null), backImageUrl ? useTexture(backImageUrl) : Promise.resolve(null)] )}
										<T.Group
											position={[0, 0, 0.01]}
											rotation={[0, 0, -rotationY * 4]}
											scale={[
												0.12 / responsiveSettings().modelScale,
												0.12 / responsiveSettings().modelScale,
												0.12 / responsiveSettings().modelScale
											]}
										>
