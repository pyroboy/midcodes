...
export class HistoryManager {
	 ...
	 */
	updateItemStatus(jobId: string, status: 'pending' | 'processing' | 'completed' | 'failed', result?: any) {
	...
	private async _loadInternal() {
		...
		try {
			...
			if (res.success) {
				...
				const remainingOptimistic = this.history.filter(
					h => h.isOptimistic && !dbIds.has(h.id)
				);
				...
				const mergedHistory = res.history.map(dbItem => {
					...
					return {
						...
						isOptimistic: wasOptimistic && (dbItem.status === 'pending' || dbItem.status === 'processing')
					};
				});
				...
				console.log(`[HistoryManager] Loaded ${mergedHistory.length} items, ${remainingOptimistic.length} optimistic remaining`);
				
				// Start polling for any pending/processing items not already being polled
				this.history.forEach(item => {
					if ((item.status === 'pending' || item.status === 'processing') 
						&& !this.pollingJobs.has(item.id)
						&& !item.id.startsWith('temp-')) { // Don't poll temp IDs
