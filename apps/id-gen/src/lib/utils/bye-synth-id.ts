	...
	for (let dy = 0; dy < blockSize && blockY + dy < height; dy++) {
		for (let dx = 0; dx < blockSize && blockX + dx < width; dx++) {
			...
			for (let freq = 1; freq <= 4; freq++) {
				dctNoise +=
					Math.cos((dx * freq * Math.PI) / blockSize) *
	...
	for (const freq of frequencies) {
		...
		for (let y = 0; y < height; y++) {
			for (let x = 0; x < width; x++) {
				...
				// Patrón de onda 2D
				const wave =
					Math.sin((x * 2 * Math.PI) / freq + phaseX) *
			...
			if (rng3.next() < intensity * 0.5) {
				...
				for (let dy = 0; dy < 16 && blockY + dy < height; dy++) {
					for (let dx = 0; dx < 16 && blockX + dx < width; dx++) {
						...
						data[idx + 1] = clamp(
							data[idx + 1] + blockNoise + rng3.nextSigned() * intensity * 2,
							0,
							255
						);
						data[idx + 2] = clamp(
							data[idx + 2] + blockNoise + rng3.nextSigned() * intensity * 2,
							0,
							255
						);
					}
 ...
 */
export const restoreImageLSB = (processedImageData: ImageData, _seed: string): ImageData => {
		...
		canvas.toBlob(
			(blob) => {
				if (blob) {
					...
					if (format === 'jpeg') {
						recompressJPEG(blob, quality * 0.95)
							.then(resolve)
							.catch(() => resolve(blob));
					} else {
						resolve(blob);
