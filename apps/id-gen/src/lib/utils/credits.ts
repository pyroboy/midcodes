 ...
 */
export async function canGenerateCard(userId: string): Promise<{ canGenerate: boolean; needsCredits: boolean }> {
  const credits = await getUserCredits(userId);
 ...
 */
export async function incrementTemplateCount(userId: string): Promise<{ success: boolean; newCount: number }> {
  try {
    const credits = await getUserCredits(userId);
    if (!credits) {
      throw new Error('User not found');
 ...
 */
async function createCreditTransaction(transaction: Omit<CreditTransaction, 'id' | 'created_at' | 'updated_at'>): Promise<CreditTransaction | null> {
  const { data, error } = await supabase
    .from('credit_transactions')
    .insert(transaction)
    .select()
