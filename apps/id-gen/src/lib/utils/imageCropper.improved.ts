...
function validateImageFile(file: File): { valid: boolean; error?: string } {
	...
	if (file.size > maxSize) {
		return {
			valid: false,
			error: `File too large: ${file.size} bytes. Maximum allowed: ${maxSize} bytes`
		};
	}
...
function validateInputs(
	...
	if (!originalImageSize || originalImageSize.width <= 0 || originalImageSize.height <= 0) {
		return {
			valid: false,
			error: `Invalid original image dimensions: ${JSON.stringify(originalImageSize)}`
		};
	}
	...
	if (
		!backgroundPosition ||
		typeof backgroundPosition.x !== 'number' ||
		!isFinite(backgroundPosition.x) ||
		typeof backgroundPosition.y !== 'number' ||
		!isFinite(backgroundPosition.y) ||
		typeof backgroundPosition.scale !== 'number' ||
		!isFinite(backgroundPosition.scale) ||
		backgroundPosition.scale <= 0
	) {
		return {
			valid: false,
			error: `Invalid background position: ${JSON.stringify(backgroundPosition)}`
		};
	}
...
export function needsCropping(
	...
	try {
		...
		const isOffCenter =
			Math.abs(backgroundPosition.x) > 0.1 || Math.abs(backgroundPosition.y) > 0.1;
...
function calculateCropArea(
	...
	try {
		...
		console.log('📏 Scale calculations:', {
			...
			scaleY,
			s0,
			userScale: backgroundPosition.scale
		});
...
export async function cropBackgroundImage(
	...
	return new Promise((resolve, reject) => {
		...
		img.onload = () => {
			...
			try {
				...
				if (originalSize.width <= 0 || originalSize.height <= 0) {
					throw new Error(
						`Invalid loaded image dimensions: ${originalSize.width}x${originalSize.height}`
					);
				}
				...
				if (wasCropped) {
					...
					const sWidth = Math.max(
						1,
						Math.min(cropArea.sourceWidth, originalSize.width - cropArea.sourceX)
					);
					const sHeight = Math.max(
						1,
						Math.min(cropArea.sourceHeight, originalSize.height - cropArea.sourceY)
					);
					...
					const dWidth = Math.max(
						1,
						Math.min(cropArea.destWidth, templateSize.width - cropArea.destX)
					);
					const dHeight = Math.max(
						1,
						Math.min(cropArea.destHeight, templateSize.height - cropArea.destY)
					);

					console.log('🎨 Final draw parameters:', [
						sx,
						sy,
						sWidth,
						sHeight,
		...
		img.onerror = (errorEvent) => {
			...
			reject(
				new Error(
					`Failed to load image: ${imageFile.name}. The image file may be corrupted or in an unsupported format.`
				)
			);
		};
