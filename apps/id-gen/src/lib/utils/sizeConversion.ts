 ...
 */
export function formatDimensions(cardSize: CardSize, showPixels: boolean = true, dpi: number = DEFAULT_DPI): string {
 ...
 */
export function pixelsToCardSize(dimensions: PixelDimensions, unit: UnitType = 'inches', dpi: number = DEFAULT_DPI): CardSize {
    return {
        name: 'Custom',
 ...
 */
export function findClosestCardSize(dimensions: PixelDimensions, dpi: number = DEFAULT_DPI): CardSize {
    ...
    for (const size of COMMON_CARD_SIZES) {
        ...
        const sizeInches = size.unit === 'inches' ? size : {
...
export function validateCardDimensions(cardSize: CardSize): { valid: boolean; error?: string } {
    ...
    const widthInches = cardSize.unit === 'inches' ? cardSize.width : 
        unitToPixels(cardSize.width, cardSize.unit) / DEFAULT_DPI;
    const heightInches = cardSize.unit === 'inches' ? cardSize.height :
 ...
 */
export function createCardSizeFromPixels(width: number, height: number, name: string = 'Legacy'): CardSize {
    return {
        name,
        width,
        height,
