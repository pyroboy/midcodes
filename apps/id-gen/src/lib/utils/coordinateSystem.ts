...
export class CoordinateSystem {
    ...
    createPositionStyle(storageX: number, storageY: number, storageWidth: number, storageHeight: number): Record<string, string> {
        const previewPos = this.storageToPreview({ x: storageX, y: storageY });
        const previewDims = this.storageToPreviewDimensions({ width: storageWidth, height: storageHeight });
