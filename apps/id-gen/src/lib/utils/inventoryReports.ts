...
export function exportSupplierPerformanceReport(data: SupplierPerformanceData[]): string {
	const headers = ['Supplier', 'Total Orders', 'On-Time Deliveries', 'Quality Score', 'Performance Score', 'Avg Delivery Days'];
...
export function exportReorderReport(data: any[]): string {
	const headers = ['SKU', 'Name', 'Current Qty', 'Reorder Level', 'Suggested Order Qty', 'Supplier'];
	const rows = data.map(item => [
		item.sku || '',
		item.name || '',
