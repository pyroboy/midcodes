...
function calculateScaleFactors(
    ...
    switch (strategy) {
        ...
        case 'uniform':
            ...
            const avgScale = (canvasWidth / LEGACY_DIMENSIONS.width + canvasHeight / LEGACY_DIMENSIONS.height) / 2;
...
export function previewAdaptiveElements(
    ...
    console.log(`Creating adaptive elements for ${canvasWidth}×${canvasHeight} canvas (${side} side)`);
    ...
    adaptiveElements.forEach((element, index) => {
        console.log(`  ${index + 1}. ${element.id}: ${element.x},${element.y} (${element.width}×${element.height})`);
    });
