...
import { supabaseAdmin, type PaymentRecord, type PaymentRecordInsert, type PaymentRecordUpdate, type WebhookEvent, type WebhookEventInsert } from '$lib/server/supabase';
 ...
 */
export async function getPaymentByProviderId(providerPaymentId: string): Promise<PaymentRecord | null> {
  const { data, error } = await supabaseAdmin
    .from('payment_records')
    .select('*')
    .eq('provider_payment_id', providerPaymentId)
...
export async function listPaymentsByUser({
  ...
  limit = 50,
  cursor
}: ListPaymentsByUserParams): Promise<{ data: PaymentRecord[]; hasMore: boolean; nextCursor?: string }> {
