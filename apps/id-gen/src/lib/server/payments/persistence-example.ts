 ...
 */
export async function exampleUpdateWithProviderPaymentId(sessionId: string, providerPaymentId: string) {
  try {
...
export async function exampleProcessFailedPaymentWebhook(webhookPayload: any) {
  const eventId = webhookPayload.data.id;
  const providerPaymentId = webhookPayload.data.attributes.payment_intent_id;
  const failureReason = webhookPayload.data.attributes.last_payment_error?.message || 'Payment failed';
