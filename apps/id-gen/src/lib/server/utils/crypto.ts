 ...
 */
export function verifyPayMongoWebhook(signatureHeader: string, rawBody: string, webhookSecret: string): boolean {
  if (!signatureHeader) {
    return false;
  }
  ...
  return timingSafeEqual(Buffer.from(signatureToVerify, 'hex'), Buffer.from(expectedSignature, 'hex'));
}
