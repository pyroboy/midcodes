...
<div class="min-h-screen bg-background">
	{#if data.blockedByEmulation}
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-95 p-4 text-center">
			<div class="max-w-md w-full bg-gray-800 rounded-lg shadow-xl p-8 border border-gray-700">
				<div class="mb-6">
					<div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-blue-900 mb-4">
					...
					<h2 class="text-2xl font-bold text-white mb-2">Viewing as {data.roleEmulation?.emulatedRole}</h2>
	...
	{:else}
		{#if data.warningEmulationIgnored && !dismissedWarning}
			<div class="bg-amber-100 border-b border-amber-200 text-amber-900 px-4 py-3 flex items-center justify-between">
				<div class="flex items-center">
					...
					<span class="text-sm font-medium">
						Emulation Active: You are viewing this page as your original role ({data.roleEmulation?.originalRole}) because your emulated role ({data.roleEmulation?.emulatedRole}) does not have access.
					</span>
				</div>
				<div class="flex items-center space-x-3">
					<button onclick={stopEmulation} class="text-sm font-bold underline hover:text-amber-700">Stop Emulating</button>
					<button onclick={() => dismissedWarning = true} class="text-amber-500 hover:text-amber-700">
				...
				<div class="flex justify-between items-center h-16">
					...
					<div class="flex items-center space-x-4">
						{#if data.roleEmulation?.active}
							<div class="hidden md:flex items-center bg-blue-900 bg-opacity-50 text-blue-200 px-3 py-1 rounded-full text-xs border border-blue-800 mr-2">
								<span class="mr-2">Viewing as {data.roleEmulation.emulatedRole}</span>
								<button onclick={stopEmulation} class="hover:text-white font-bold underline">Exit</button>
							</div>
						{/if}
