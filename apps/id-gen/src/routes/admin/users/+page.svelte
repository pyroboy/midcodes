<script lang="ts">
	...
	const roleOptions = [
		...
		{ value: 'id_gen_admin', label: 'ID Generator Admin', description: 'Template and user management' },
	...
	function canDeleteUser(user: any, currentUserRole?: string, currentUserId?: string, users: any[] = []) {
					...
					{#each filteredUsers as user}
						<div class="border rounded-lg p-4 space-y-3">
							...
							<div class="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
								...
								<div>
									<span class="font-medium">Active:</span> {formatDate(user.updated_at)}
								</div>
							</div>
...
<Dialog.Root bind:open={showAddUserDialog}>
	<Dialog.Content>
		...
		<div class="flex justify-end gap-2 mt-6">
			<Button type="button" variant="outline" onclick={() => { showAddUserDialog = false; }}>
				Cancel
			</Button>
...
{#if selectedUser}
	<Dialog.Root bind:open={showEditUserDialog}>
		<Dialog.Content>
			...
			<div class="flex justify-end gap-2 mt-6">
				<Button type="button" variant="outline" onclick={() => { showEditUserDialog = false; }}>
					Cancel
				</Button>
				<Button onclick={() => handleUpdateUserRole(selectedUser.id, selectedUser.role)} disabled={loading}>
					{#if loading}
						<svg class="animate-spin -ml-1 mr-3 h-4 w-4 text-white" fill="none" viewBox="0 0 24 24">
							<circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
							<path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
...
{#if selectedUser}
	<Dialog.Root bind:open={showDeleteConfirmDialog}>
		<Dialog.Content>
			...
			<div class="flex justify-end gap-2 mt-6">
				<Button type="button" variant="outline" onclick={() => { showDeleteConfirmDialog = false; }}>
					Cancel
				</Button>
