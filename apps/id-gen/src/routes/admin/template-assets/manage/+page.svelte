<script lang="ts">
	...
	const categories = $derived([...new Set((data.assets as unknown as TemplateAsset[]).map(a => a.category).filter(Boolean))]);
...
<div class="container mx-auto max-w-7xl px-4 py-8">
	...
	<div class="mb-8 flex items-center justify-between">
		<div>
			...
			<p class="mt-1 text-muted-foreground">
				View and manage uploaded template assets
			</p>
		</div>
        <a 
            href="/admin/template-assets" 
	...
	<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
		{#each filteredAssets as asset (asset.id)}
			<div class="group relative overflow-hidden rounded-lg border border-border bg-card transition-shadow hover:shadow-md" transition:fade>
				<!-- Image -->
				...
				<div class="p-4">
					<div class="mb-2 flex items-start justify-between gap-2">
                        <div>
                            ...
                            <p class="text-xs text-muted-foreground capitalize">{asset.orientation} • {asset.sample_type.replace('_', ' ')}</p>
                        </div>
						<span class={asset.is_published ? "h-2 w-2 rounded-full bg-green-500" : "h-2 w-2 rounded-full bg-yellow-500"} title={asset.is_published ? "Published" : "Draft"}></span>
					</div>
