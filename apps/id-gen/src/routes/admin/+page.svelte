	...
	<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
		...
		<Card>
			<CardHeader class="flex flex-row items-center justify-between space-y-0 pb-2">
				...
				<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4V2a1 1 0 011-1h8a1 1 0 011 1v2h4a1 1 0 011 1v16a1 1 0 01-1 1H3a1 1 0 01-1-1V5a1 1 0 011-1h4zM9 4h6V3H9v1zm-2 3v12h10V7H7z" />
				</svg>
			</CardHeader>
			<CardContent>
				{#await dashboardData}
		...
		<Card>
			...
			<CardContent>
				...
				{:then data}
					...
					{@const userStats = {
						total: users.length,
						admins: users.filter(u => u.role && ['super_admin', 'org_admin', 'id_gen_admin'].includes(u.role)).length,
		...
		<Card>
			...
			<CardContent class="space-y-2">
				...
				{:then billingData}
					<div class="text-2xl font-bold">{billingData?.payments_enabled ? 'Enabled' : 'Disabled'}</div>
					<p class="text-xs text-muted-foreground">
				...
				<div class="mt-2 space-y-2">
					...
					{:then billingData}
						<Button
							...
							onclick={() => { desiredEnabled = !billingData?.payments_enabled; onTogglePayments(); }}
						>
