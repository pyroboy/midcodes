<script lang="ts">
	...
	// UI Icons
	import { 
		Layers, Wand2, Loader2, Check, Circle, Eye, EyeOff, Merge, 
		Sparkles, ChevronUp, ChevronDown, ZoomIn, Eraser, Crop, ImageDown, ArrowUp, ArrowDown 
	} from 'lucide-svelte';
        ...
        if (actionModalLayerId) {
            ...
            if (layer) {
                await processor.upscaleLayer(layer, settings.upscaleModel, settings.removeWatermark);
                 ...
                 const res = await processor.upscaleLayer(newLayer as any, settings.upscaleModel, settings.removeWatermark);
                 if (res) {
...
<div class="container mx-auto max-w-7xl px-4 py-6">
	...
	<div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
		<div class="lg:col-span-2 space-y-4">
			...
			<div class="p-4 rounded-lg border border-border bg-card space-y-4 flex justify-between items-center">
				<div class="flex items-center gap-4">
					<Label class="text-sm text-muted-foreground">Default Layers:</Label>
					<select bind:value={settings.numLayers} class="rounded border border-border bg-background px-2 py-1 text-sm h-8">
						{#each [2,3,4,6,8] as n}<option value={n}>{n}</option>{/each}
					</select>
				</div>
                <div class="flex items-center gap-2">
                    <Button variant="outline" size="sm" onclick={() => handleMenuAction('crop', 'original-file')}>
			...
			<div class="rounded-xl border border-border bg-card shadow-sm flex flex-col h-[500px]">
				<div class="p-4 border-b border-border flex justify-between items-center shrink-0">
					<div class="flex items-center gap-3">
						...
						<div>
							<h2 class="font-semibold text-foreground">Layers ({layerMgr.currentLayers.length})</h2>
							<div class="flex items-center gap-2 text-sm">
								{#if layerMgr.saveState === 'saved'}
									<span class="flex items-center gap-1 text-green-600"><Check class="h-3 w-3" /> Saved</span>
								{:else}
									<span class="flex items-center gap-1 text-yellow-600"><Circle class="h-3 w-3" /> Unsaved</span>
								{/if}
							</div>
						</div>
					</div>
					<div class="flex gap-1">
						<Button variant="ghost" size="icon" onclick={() => layerMgr.mergeMode = !layerMgr.mergeMode}>
				...
				{#if layerMgr.mergeMode}
					<div class="p-2 bg-violet-500/10 border-b border-violet-500/20 flex justify-between items-center px-4 shrink-0">
						<span class="text-xs text-violet-700">{layerMgr.selectedForMerge.size} selected</span>
						<Button size="sm" class="h-7 text-xs bg-violet-600" onclick={() => processor.mergeSelectedLayers()}>
							Merge
						</Button>
					</div>
				{/if}
				...
				<div class="flex-1 overflow-y-auto p-2 space-y-2" ondrop={handleDrop} ondragover={(e)=>e.preventDefault()} role="region" aria-label="Layer list">
					...
					{#each layerMgr.currentLayers as layer (layer.id)}
						<LayerCard
                            ...
                            onUpdateVariableName={(name) => layerMgr.updateSelection(layer.id, { variableName: name })}
                            onUpdateType={(type) => layerMgr.updateSelection(layer.id, { elementType: type })}
                            onSetOpacity={(val) => layerMgr.setOpacity(layer.id, val)}
                            onToggleIncluded={() => layerMgr.updateSelection(layer.id, { included: !layerMgr.selections.get(layer.id)?.included })}
	...
	<RemoveModal
		...
		bind:removePrompt={removePrompt}
        isProcessing={processor.isProcessing}
		onProceed={async () => {
			modals.remove = false;
    ...
    <ImagePreviewModal
        ...
        assetDimensions={{ width: data.asset.widthPixels || 1050, height: data.asset.heightPixels || 600 }}
    />
