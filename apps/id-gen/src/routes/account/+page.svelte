<script lang="ts">
	...
	function getTransactionTypeColor(type: string): string {
		switch (type) {
			...
			case 'usage': return 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300';
			case 'refund': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300';
			case 'bonus': return 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300';
	...
	function getTransactionTypeIcon(type: string): string {
		switch (type) {
			case 'purchase': return 'M12 6v6m0 0v6m0-6h6m-6 0H6';
			case 'usage': return 'M20 12H4';
			case 'refund': return 'M16 15v-1a4 4 0 00-4-4H8m0 0l3 3m-3-3l3-3m9 14V9a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2z';
			case 'bonus': return 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z';
...
<svelte:head>
	...
	<meta name="description" content="Manage your accounts, view credits balance, and track your transaction history." />
</svelte:head>
...
<div class="container mx-auto px-4 py-8 max-w-6xl">
	...
	<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
		{#await getDashboardStats()}
			...
			{/each}
		{:then stats}
			<DashboardStatsCard title="Total IDs Created" value={stats.totalCards} icon="id-card" href="/all-ids" />
			<DashboardStatsCard title="Templates Available" value={stats.totalTemplates} icon="layout-template" href="/templates" />
			<DashboardStatsCard title="Recent Activity" value={stats.recentCardsCount} icon="trending-up" />
	...
	<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
		{#await getUserCredits()}
			<!-- Credits Skeleton -->
			{#each Array(3) as _, i}
				<Card class="animate-pulse {i === 0 ? 'bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20' : i === 1 ? 'bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20' : 'bg-gradient-to-br from-purple-50 to-violet-50 dark:from-purple-900/20 dark:to-violet-900/20'}">
		...
		{:then userCredits}
			<!-- Credits Balance -->
			<Card class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border-blue-200 dark:border-blue-800">
				<CardContent class="p-6">
			...
			<!-- Cards Generated -->
			<Card class="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border-green-200 dark:border-green-800">
				<CardContent class="p-6">
					<div class="flex items-center justify-between">
						<div>
			...
			<!-- Templates Created -->
			<Card class="bg-gradient-to-br from-purple-50 to-violet-50 dark:from-purple-900/20 dark:to-violet-900/20 border-purple-200 dark:border-purple-800">
				<CardContent class="p-6">
					<div class="flex items-center justify-between">
						<div>
							<p class="text-sm font-medium text-purple-600 dark:text-purple-400">Templates Created</p>
		...
		<Card>
			...
			<CardContent class="space-y-4">
				{#await getUserCredits()}
					<div class="space-y-3">
						{#each Array(2) as _}
							<div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg animate-pulse">
					...
					<div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
						<div class="flex items-center gap-3">
							...
							<div>
								...
								<p class="text-sm text-gray-600 dark:text-gray-400">Create unlimited custom templates</p>
							</div>
						</div>
						<div>
							{#if userCredits?.unlimited_templates}
								<Badge class="bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300">Active</Badge>
							{:else if !$paymentFlags.paymentsEnabled}
								<div class="text-xs text-gray-500 dark:text-gray-400 text-center max-w-32">Contact admin</div>
							{:else}
		...
		<Card>
			...
			<CardContent class="space-y-3">
				{#if !$paymentFlags.paymentsEnabled}
					<div class="w-full px-3 py-2 text-center text-sm text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 rounded-md">
	...
	<Card>
		...
		<CardContent>
			{#await getCreditHistory()}
				...
				<div class="space-y-3">
					{#each Array(5) as _}
						<div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg animate-pulse">
							<div class="flex items-center gap-3">
								<div class="h-6 w-16 bg-muted rounded"></div>
								<div>
									<div class="h-4 w-32 bg-muted rounded mb-1"></div>
			...
			{:then creditHistory}
										...
										<td class="px-4 py-4 whitespace-nowrap">
											<Badge class="{getTransactionTypeColor(transaction.transaction_type)} flex items-center gap-1 w-fit">
												<svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
													<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={getTransactionTypeIcon(transaction.transaction_type)}/>
												</svg>
				...
				{:else}
					<div class="text-center py-12">
						...
						<p class="text-gray-500 dark:text-gray-400 mb-4">Your credit purchases and usage will appear here.</p>
						{#if !$paymentFlags.paymentsEnabled}
							<div class="px-4 py-2 text-center text-sm text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 rounded-md inline-block">
