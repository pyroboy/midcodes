		...
		{#if searchSuggestions.length > 0 && searchQuery}
			<div class="search-suggestions">
				{#each searchSuggestions as suggestion}
					<button class="suggestion-item" onclick={() => (searchQuery = suggestion.query)}>
		...
		{#if searchHistory.length > 0 && !searchQuery && inputFocused}
			<div class="search-history">
				...
				{#each searchHistory.slice(0, 5) as historyItem}
					<button class="history-item" onclick={() => (searchQuery = historyItem.query)}>
	...
	{#if searchResults.length > 0}
		<div class="search-summary">
			...
			<div class="search-actions-secondary">
				...
				<button onclick={exportSearchResults} class="export-results-btn"> Export Results </button>
			</div>
		</div>
	{/if}
</div>
	...
	<div class="saved-filters">
		...
		<div class="saved-filter-list">
			{#each savedFilters as savedFilter}
				<button class="saved-filter-item" onclick={() => applySavedFilter(savedFilter)}>
		...
		{#if filters.length > 0}
			<button class="save-current-filter" onclick={showSaveFilterDialog}>
				Save Current Filter
			</button>
		{/if}
	</div>
