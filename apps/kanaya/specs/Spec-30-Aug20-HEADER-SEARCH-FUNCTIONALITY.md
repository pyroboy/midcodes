		...
		<div class="fixed top-20 left-1/2 transform -translate-x-1/2 w-full max-w-lg mx-4">
			...
			>
				...
				<div class="flex items-center p-4 border-b border-gray-200 dark:border-gray-700">
					...
					/>
					<kbd
						class="hidden sm:inline-flex h-5 px-1 bg-gray-100 dark:bg-gray-700 rounded text-xs text-gray-500"
					>
						ESC
					</kbd>
				</div>
				...
				<!-- Footer -->
				<div
					class="flex items-center justify-between p-3 border-t border-gray-200 dark:border-gray-700 text-xs text-gray-500"
				>
...
>
	...
	<kbd class="ml-auto px-1 bg-white dark:bg-gray-700 rounded text-xs"> ⌘K </kbd>
</button>

<!-- SearchButton.svelte - Mobile version -->
<Button variant="ghost" size="icon" onclick={openSearch} aria-label="Search" class="sm:hidden">
...
export async function searchIdCards(query: string): Promise<SearchItem[]> {
	const { data: idCards } = await supabase.from('idcards').select('id, card_data').limit(3);
