...
<div
	class="error-display border rounded-lg p-4 {getErrorStyles(error.severity)}"
	role="alert"
	aria-live="polite"
>
		...
		<div class="flex-1">
			...
			{#if error.technicalMessage && __DEV__}
				<details class="mt-2">
					...
					<pre
						class="text-xs mt-1 p-2 bg-black/10 rounded overflow-x-auto">{error.technicalMessage}</pre>
				</details>
			{/if}
		</div>
	...
	private getToastDuration(severity: ErrorSeverity): number {
		switch (severity) {
			...
			case ErrorSeverity.ERROR:
				return 7000;
			case ErrorSeverity.CRITICAL:
				return Infinity; // Requires manual dismissal
		}
	}
}
```
...
<script lang="ts">
	...
	function validateField(fieldName: string, value: any) {
		try {
			...
			formErrors[fieldName] = errorHandler.createValidationError(fieldName, error.message);
			formErrors = { ...formErrors };
		}
	}
	...
	async function submitForm() {
		try {
			...
			await retryManager.withRetry(() => api.submitForm(formData), {
...
<form onsubmit={submitForm} class="space-y-4">
	...
	<button type="submit" disabled={Object.keys(formErrors).length > 0}> Submit Form </button>
</form>
```
