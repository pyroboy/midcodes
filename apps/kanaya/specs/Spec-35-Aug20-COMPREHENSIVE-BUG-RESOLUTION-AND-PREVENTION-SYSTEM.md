...
```typescript
async function safeSupabaseOperation<T>(operation: () => Promise<{ data: T | null; error: any }>) {
...
async function handlePurchase(packageId: string) {
	const result = await safeAsync(() => paymentService.createPayment(packageId));
...
throw new ApplicationError('Failed to load template', 'network', true, { templateId, userId });
```
...
// package.json husky setup
{
	...
	"lint-staged": {
		"*.{ts,svelte}": ["eslint --fix", "prettier --write", "check-for-debug-code"]
	}
}
```
