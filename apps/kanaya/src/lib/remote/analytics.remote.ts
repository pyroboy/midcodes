...
export const getOverviewAnalytics = query(async (): Promise<OverviewStats> => {
	...
	const [totalCardsResult, cardsThisMonthResult, users, paidInvoices] = await Promise.all([
		// Total cards
		db
		...
		db
			.select({ count: sql`count(*)` })
			.from(idcards)
			.where(and(eq(idcards.orgId, org_id), gte(idcards.createdAt, thisMonth))),
		...
		// Paid invoices
		db
			.select({ totalAmount: invoices.totalAmount, paidAt: invoices.paidAt })
			.from(invoices)
...
export const getRevenueAnalytics = query(
	'unchecked',
	async ({ days = 30 }: { days?: number }): Promise<RevenueAnalytics> => {
		const { org_id } = await requireAdminPermissions();
		if (!org_id) throw error(500, 'Org ID missing');
...
export const getCreditAnalytics = query(
	'unchecked',
	async ({ days = 30 }: { days?: number }): Promise<CreditAnalytics> => {
		const { org_id } = await requireAdminPermissions();
		if (!org_id) throw error(500, 'Org ID missing');

		const { start } = getDateRange(days);

		const [txList, usersList] = await Promise.all([
		...
		txList.forEach((tx) => {
			switch (tx.transactionType) {
				...
				case 'usage':
					...
					break;
				case 'refund':
					totalRefunded += Math.abs(tx.amount);
					break;
				case 'bonus':
...
export const getCardAnalytics = query(
	'unchecked',
	async ({ days = 30 }: { days?: number }): Promise<CardAnalytics> => {
		const { org_id } = await requireAdminPermissions();
		if (!org_id) throw error(500, 'Org ID missing');
		...
		const [totalCardsResult, cardsThisMonthResult, recentCards, templatesList, usersList] =
			await Promise.all([
				db
					.select({ count: sql`count(*)` })
					.from(idcards)
				...
				db
					...
					.where(and(eq(idcards.orgId, org_id), gte(idcards.createdAt, thisMonth))),
				db
					.select({ id: idcards.id, templateId: idcards.templateId, createdAt: idcards.createdAt })
					.from(idcards)
...
export const getUserAnalytics = query(
	'unchecked',
	async ({ days = 30 }: { days?: number }): Promise<UserAnalytics> => {
		const { org_id } = await requireAdminPermissions();
		if (!org_id) throw error(500, 'Org ID missing');
		...
		const activeUsers = usersList.filter((u) => u.updatedAt && u.updatedAt >= thirtyDaysAgo).length;

		// New users this month
		const newUsersThisMonth = usersList.filter(
			(u) => u.createdAt && u.createdAt >= thisMonth
		).length;
