	...
	try {
		...
		const [
			...
			db
				.select({ count: count() })
				.from(schema.idcards)
				.where(and(eq(schema.idcards.orgId, org_id), gte(schema.idcards.createdAt, startOfToday))),
			...
			// Credit stats
			db
				.select({ credits_balance: schema.profiles.creditsBalance })
				.from(schema.profiles)
				.where(eq(schema.profiles.orgId, org_id)),
			...
			// Revenue
			db
				.select({ total_amount: schema.paymentRecords.amountPhp }) // Corrected to paymentRecords
		...
		const totalRevenue =
			paidInvoices.reduce((sum, inv) => sum + Number(inv.total_amount || 0), 0) || 0;
		const paidInvoicesCount = paidInvoices.length || 0;

		// Calculate new cards this month
		const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
		const newCardsThisMonth =
			recentCardsData.filter((card) => card.created_at && card.created_at >= startOfMonth).length ||
			0;

		// Create recent activity
		const recentActivity = recentCardsData
			.map((card) => ({
				id: card.id,
...
export const bulkAssignPermissions = command('unchecked', async ({ role, permissions }: any) => {
	await requireSuperAdminPermissions();
...
export const getOrganizationDetails = query(async () => {
	...
	try {
		const [organization, members, [totalCardsResult], [totalTemplatesResult], orgSettings] =
			await Promise.all([
...
export const removeOrganizationMember = command('unchecked', async ({ memberId }: any) => {
	...
	try {
		...
		const [member] = await db
			.select({ id: schema.profiles.id, email: schema.profiles.email, role: schema.profiles.role })
			.from(schema.profiles)
			.where(and(eq(schema.profiles.id, memberId), eq(schema.profiles.orgId, org_id)))
			.limit(1);
...
export const updateOrganizationSettings = command(
	'unchecked',
	async ({ paymentsEnabled, paymentsBypass }: any) => {
		const { user, org_id } = await requireUserManagementPermissions();
