...
export function computeVisibleRectInImage(
	...
	if (
		!isFinite(image.width) ||
		!isFinite(image.height) ||
		!isFinite(container.width) ||
...
export function mapImageRectToThumb(rectInImage: Rect, image: Dims, thumb: Dims): Rect {
	...
	if (
		!isFinite(image.width) ||
		!isFinite(image.height) ||
		!isFinite(thumb.width) ||
		!isFinite(thumb.height) ||
		!isFinite(rectInImage.x) ||
		!isFinite(rectInImage.y) ||
		!isFinite(rectInImage.width) ||
		!isFinite(rectInImage.height) ||
		image.width <= 0 ||
		image.height <= 0 ||
		thumb.width <= 0 ||
		thumb.height <= 0
...
export function validateCropFrameAlignment(
	...
	return (
		Math.abs(visibleRect.width - container.width) < tolerance &&
