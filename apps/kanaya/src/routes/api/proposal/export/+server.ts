...
const proposalSchema = z.object({
	mode: z.literal('form').optional().default('form'),
...
const markdownSchema = z.object({
	...
	orgName: z.string().optional().default('Client')
});
...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (body.mode === 'markdown') {
		...
		const pdfBytes = await generateMarkdownPdf(data.markdown, data.orgName);
		return new Response(pdfBytes as any, {
			headers: {
				'content-type': 'application/pdf',
				'content-disposition': `attachment; filename="kanaya-proposal-${data.orgSlug}.pdf"`
			}
		});
	}

	// Default: form-based generation
...
async function generateMarkdownPdf(markdown: string, orgName: string): Promise<Uint8Array> {
	const doc = await PDFDocument.create();
	const fontBold = await doc.embedFont(StandardFonts.HelveticaBold);
	const fontRegular = await doc.embedFont(StandardFonts.Helvetica);
	...
	const colors = {
		primary: rgb(43 / 255, 108 / 255, 176 / 255),
		dark: rgb(26 / 255, 32 / 255, 44 / 255),
		muted: rgb(113 / 255, 128 / 255, 150 / 255),
		border: rgb(209 / 255, 213 / 255, 219 / 255),
		headerBg: rgb(31 / 255, 41 / 255, 55 / 255),
		rowAlt: rgb(243 / 255, 244 / 255, 246 / 255),
		white: rgb(1, 1, 1)
	};
	...
	page.drawText('KANAYA', { x: margin, y: pageHeight - 55, size: 32, font: fontBold, color: colors.white });
	page.drawText('IDENTITY SOLUTIONS', { x: margin, y: pageHeight - 75, size: 12, font: fontRegular, color: rgb(190 / 255, 227 / 255, 248 / 255) });
	page.drawText(`Proposal for ${orgName}`, { x: margin, y: pageHeight - 100, size: 14, font: fontRegular, color: colors.white });
	y = pageHeight - 150;
	...
	for (const line of lines) {
		...
		if (trimmed.startsWith('# ')) {
			...
			y -= 20;
			checkOverflow(30);
			page.drawText(text, { x: margin, y, size: 20, font: fontBold, color: colors.primary });
			y -= 28;
			page.drawLine({ start: { x: margin, y: y + 8 }, end: { x: margin + 50, y: y + 8 }, thickness: 3, color: colors.primary });
			continue;
		}
		...
		if (trimmed.startsWith('## ')) {
			...
			y -= 16;
			checkOverflow(24);
			page.drawRectangle({ x: margin, y: y - 2, width: 4, height: 18, color: colors.primary });
			page.drawText(text, { x: margin + 14, y, size: 16, font: fontBold, color: colors.dark });
			y -= 24;
			continue;
		}
		...
		if (trimmed.startsWith('### ')) {
			...
			y -= 10;
			checkOverflow(18);
			page.drawText(text, { x: margin, y, size: 12, font: fontBold, color: colors.dark });
			y -= 18;
			continue;
		}
		...
		if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
			...
			const wrapped = wrapText(text, fontRegular, 10, contentWidth - 20);
			checkOverflow(15);
			page.drawCircle({ x: margin + 5, y: y + 3, size: 2, color: colors.primary });
			for (const wl of wrapped) {
				checkOverflow(15);
				page.drawText(wl, { x: margin + 15, y, size: 10, font: fontRegular, color: colors.dark });
				y -= 14;
			}
			continue;
		}

		// Table row (simple: | col | col | col |)
		if (trimmed.startsWith('|') && trimmed.endsWith('|')) {
			...
			const isHeader = lines.indexOf(line) >= 0 &&
				lines[lines.indexOf(line) + 1]?.trim().includes('---');
			const colWidth = contentWidth / cells.length;

			checkOverflow(20);
			if (isHeader) {
				page.drawRectangle({ x: margin, y: y - 4, width: contentWidth, height: 18, color: colors.headerBg });
			}

			for (let i = 0; i < cells.length; i++) {
				const cellFont = isHeader ? fontBold : fontRegular;
				const cellColor = isHeader ? colors.white : colors.dark;
				const cellText = cells[i].length > 40 ? cells[i].slice(0, 38) + '...' : cells[i];
		...
		// Bold text prefix (e.g. **Date:** value)
		const boldMatch = trimmed.match(/^\*\*(.+?)\*\*\s*(.*)/);
		if (boldMatch) {
			const [, boldPart, rest] = boldMatch;
			const fullText = `${boldPart} ${rest}`;
			const wrapped = wrapText(fullText, fontRegular, 10, contentWidth);
			for (const wl of wrapped) {
				checkOverflow(14);
				// Draw the bold prefix part
				const boldWidth = fontBold.widthOfTextAtSize(boldPart, 10);
				if (wl.startsWith(boldPart)) {
					page.drawText(boldPart, { x: margin, y, size: 10, font: fontBold, color: colors.dark });
					const remainder = wl.slice(boldPart.length);
					if (remainder) {
						page.drawText(remainder, { x: margin + boldWidth, y, size: 10, font: fontRegular, color: colors.dark });
					}
				} else {
					page.drawText(wl, { x: margin, y, size: 10, font: fontRegular, color: colors.dark });
				}
				y -= 14;
			}
			continue;
		...
		const wrapped = wrapText(cleanText, fontRegular, 10, contentWidth);
		for (const wl of wrapped) {
			checkOverflow(14);
			page.drawText(wl, { x: margin, y, size: 10, font: fontRegular, color: colors.dark });
			y -= 14;
