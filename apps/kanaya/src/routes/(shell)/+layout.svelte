<script lang="ts">
	...
	// allIdsCache was in './all-ids/allIdsCache' relative to 'src/routes'.
	// Now we are in 'src/routes/(shell)'. 'all-ids' is in 'src/routes/(shell)/all-ids'.
	// So './all-ids/allIdsCache' should still work if 'all-ids' moved to (shell).
	// I moved 'all-ids' to (shell). So it is a sibling. './all-ids/allIdsCache' is correct.
							...
							<div class="flex gap-0 min-w-max">
								{#each allRolesAccess as roleInfo}
									<div
                                        ...
                                        {roleInfo.isCurrentRole && !roleInfo.isEmulatedRole
											? 'bg-blue-600/30'
											: ''}"
									>
										<!-- Access Icon -->
