...
async function verifyPaymentSystem(supabase: SupabaseClient, orgId: string) {
	...
	try {
		const { data: tableInfo, error } = await supabase.from('org_settings').select('*').limit(0); // Just check table structure
	...
	try {
		const { data: auditInfo, error } = await supabase.from('admin_audit').select('*').limit(0); // Just check table structure
	...
	try {
		const { data: txInfo, error } = await supabase.from('credit_transactions').select('*').limit(0); // Just check table structure
